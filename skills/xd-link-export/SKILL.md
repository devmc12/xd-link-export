...
11. Validate the captured artboard against the XD thumbnail component before writing final PNGs so loading masks or blank frames are retried.
12. Stage requested native artboard images under `.tmp/` first, then commit them into the page folder root only after all requested scales succeed and pass reference validation.
13. Write a single `metadata.json` file that combines page source fields and capture details.
14. Update `pages.json` only for complete page exports so failed partial captures do not become canonical output directories.
...
- Use XD thumbnail reference validation for visual readiness; do not rely on spinner color, localized labels, or pixel-content crop bounds.
